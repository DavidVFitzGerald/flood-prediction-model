...
```
{"flood_probability": 0.5339246510907424}
```
...
1. To clone the repository, use the following command:
    ```
    git clone https://github.com/DavidVFitzGerald/flood-prediction-model.git
    ```

2. Then run the two following commands to build the container and run it
    ```
...
1. Run the following command to run the script and automatically setup the environment if it's not already setup:
    ```
    uv run predict.py
    ```
